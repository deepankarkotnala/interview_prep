...
window.IR.q["10-fine-tuning"] = {
  ...
  "cards": [
    ...
    },
    {
      ...
      "simple": "Full fine-tuning updates every weight in the model. That needs enough memory to hold the weights, their gradients and the optimiser state - several times the model size - and it produces a complete new copy of the model.\n\nLoRA freezes the original weights entirely and inserts small trainable matrices alongside them. Instead of learning a full-size update, it learns a low-rank approximation of one - two thin matrices whose product has the shape of the update. Because they are thin, the trainable parameter count is a tiny fraction of the model.\n\nThree consequences that matter operationally. Training fits on far smaller hardware. The artefact is a small adapter file, megabytes rather than gigabytes, so you can version and ship it easily. And you can host one base model and swap adapters per customer or per task, which is what makes per-customer fine-tunes practical to serve.\n\nQLoRA goes further by quantising the frozen base to 4-bit (NF4) during training while the adapters still train in 16-bit, which is what lets people fine-tune large models on a single GPU.",
      "points": [
    ...
    },
    {
      ...
      "points": [
        ...
        "GRPO / RL with verifiable rewards - sample a group of answers, score them automatically, reinforce the above-average ones. Needs a reliable checker.",
