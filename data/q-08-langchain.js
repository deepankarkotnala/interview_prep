...
window.IR.q["08-langchain"] = {
  ...
  "cards": [
    ...
    },
    {
      ...
      "simple": "I choose by the amount of orchestration I need, not by which API sounds more advanced.\n\nUse an LCEL pipeline when the application is mostly data flowing through known components: retrieve, format, call a model, parse, or run independent branches in parallel. LCEL can also route conditionally, so simple branching does not automatically require a graph.\n\nUse LangChain create_agent when the main behaviour is the standard agent loop: the model chooses a tool, sees the result, and repeats until it can answer. That loop is already backed by LangGraph, so there is no reason to rebuild it node by node.\n\nUse StateGraph directly when you need custom durable workflow behaviour: extra state, long-running steps, explicit cycles, human approval that may resume later, recovery from a checkpoint, or routing that is easier to understand as named nodes and edges.\n\nThe senior choice is usually the simplest one that still makes failure handling and state obvious.",
      "say": "I choose by orchestration needs. LCEL is for composable data flow and can sequence, parallelise and route conditionally. LangChain create_agent is my default for the standard model-and-tools loop, and it already runs on LangGraph. I drop to StateGraph when I need custom durable state, explicit cycles, long-running recovery, human approval that resumes later, or routing that is clearer as named workflow steps.",
