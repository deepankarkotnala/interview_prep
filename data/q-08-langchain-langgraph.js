/* Legacy combined Topic 08 - LangChain and LangGraph.
   Kept for old direct links. The live navigation now uses separate LangChain
   and LangGraph topics. Content is still kept current. */

window.IR = window.IR || {};
window.IR.q = window.IR.q || {};

window.IR.q["08-langchain-langgraph"] = {
  "lede": "LangChain and LangGraph appear by name in GenAI job descriptions across regions. The panel usually wants two things: that you know what the abstraction is doing underneath, and that you know when to drop it. Answering only the first half reads as a course-follower.",
  ...
  "cards": [
    ...
    },
    {
      ...
      "simple": "Put `interrupt()` inside the node that needs a human decision. When execution reaches it, LangGraph pauses, the checkpointer saves the thread state, and the interrupt payload returns to your application.\n\nYour application shows that payload to a reviewer in plain language. The reviewer can approve, reject, or return edited data.\n\nWhen the answer arrives, resume the same thread with `Command(resume=...)`. That value becomes the return value of `interrupt()`, so the node can apply the decision and continue.\n\nThe wait can be long because state is durable. No web request stays open. The downstream side effect must also be idempotent so a duplicate approval cannot send the same email or payment twice.",
      "code": "from langgraph.types import Command, interrupt\n\ndef review_email(state):\n    review = interrupt({\"action\": \"send_email\", \"draft\": state[\"draft\"]})\n    if not review[\"approved\"]:\n        return {\"status\": \"rejected\"}\n    return {\"draft\": review.get(\"draft\", state[\"draft\"]), \"status\": \"approved\"}\n\ngraph = builder.compile(checkpointer=checkpointer)\ncfg = {\"configurable\": {\"thread_id\": \"case-4471\"}}\ngraph.invoke(inputs, config=cfg)  # pauses at interrupt()\n\n# ... later, after the reviewer responds ...\ngraph.invoke(Command(resume={\"approved\": True, \"draft\": edited_text}), config=cfg)",
      "say": "I put `interrupt()` inside the node that needs approval. The graph pauses there and the checkpointer persists the thread state, while the application shows the payload to a reviewer. When the person approves, rejects or edits the proposal, I resume the same thread with `Command(resume=...)`. The wait can last hours without holding a request open, and the downstream action must be idempotent.",
