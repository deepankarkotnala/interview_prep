...
window.IR.q["12-llmops"] = {
  ...
  "cards": [
    ...
    },
    {
      ...
      "simple": "I want to answer one question later: what exactly produced this result?\n\nFor a trained or fine-tuned model, I record the code commit, data snapshot, training parameters, random seed where useful, base model, adapter or checkpoint, environment, metrics and produced artifacts. A registry then records which tested artifact is approved for staging or production.\n\nFor an API-based GenAI system, the model weights are not mine, so I track the provider/model version plus the things I do control: prompt version, tool definitions, retrieval settings, embedding model, index version, evaluation-set version and measured quality, latency and cost.\n\nTools such as MLflow can store many of these runs and artifacts, but the product name is not the point. The point is reproducibility and promotion history. A production incident should let me identify the exact configuration, compare it with the previous one, and roll back the component that changed.",
      "points": [
