  ...
  "cards": [
    {
      ...
      "simple": "The OWASP Top 10 for LLM Applications is the shared vocabulary (a 2026 edition is current, and there is a separate Top 10 for Agentic Applications). Naming it gives a security-minded interviewer a common frame.\n\nThe ones that come up in practice. Prompt injection, direct and indirect, where text the model reads carries instructions. Sensitive information disclosure, where the model reveals data the user should not see - through retrieval without permission checks, or through a system prompt it repeats. Improper output handling, where you take model output and pass it into SQL, a shell or a browser without validation, which is a classic injection vulnerability with a new source. Excessive agency, where a tool can do far more than the task requires. Supply chain, which now includes models, prompts and MCP servers, not just packages. And unbounded consumption, where an attacker or a runaway loop runs up cost or exhausts capacity.\n\nThe framing that matters more than the list: the model is not a trust boundary. Everything it produces is untrusted input to whatever comes next, and everything it reads is untrusted data. Every control follows from those two sentences.",
      "points": [
    ...
    },
    {
      ...
      "points": [
        ...
        "Tools such as NeMo Guardrails, Guardrails AI, Llama Guard or cloud services (Bedrock Guardrails, Azure AI Content Safety) implement checks; you still own placement and failure actions."
      ],
      "diagram": {
